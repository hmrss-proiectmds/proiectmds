...
class PokerEngine(GameEngine):
    ...
    def _extract_showdown(game: TexasHoldEm, log: list[str]) -> dict | None:
        ...
        try:
            ...
            for pot_id, (amount, hand_rank, winner_ids) in pot_winners.items():
                for wid in winner_ids:
                    ...
                    if seat not in winner_map:
                        # Only take the first 2 cards (hole cards) — the
                        # texasholdem library may include community cards
                        winner_map[seat] = {
                            "seat": seat,
                            "cards": [_card_str(c) for c in hand_cards[:2]],
                            "hand_name": hand_name,
                            "amount_won": 0
                        }
            ...
            # Reveal all hole hands that went to showdown (2 hole cards only)
            revealed_hands = {}
            for pid, cards in game.hands.items():
                revealed_hands[pid + 1] = [_card_str(c) for c in cards[:2]]
